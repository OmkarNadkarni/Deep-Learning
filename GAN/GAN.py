...
def generator(x):
    ...
    # 1st hidden layer
    w0 = tf.get_variable('G_w0', [x.get_shape()[1], 256], initializer=w_init) #128 original
...
def show_result(num_epoch, show=False, save=False, path='result.png'):
    ...
    with tf.variable_scope("G"):
        ...
        new_z = tf.cast(tf.convert_to_tensor(z_), tf.float32)
        gen_img = generator(new_z)
        imgs = sess.run(gen_img,{z: z_})

        for i in range(25):
            sub = fig.add_subplot(5,5,i+1)
            image_new = np.reshape(imgs[i, :], (28, 28))
            sub.imshow(image_new,cmap='gray')
            plt.axis('off')
        fig.savefig(path,cmap='gray')
        plt.close()
...
for epoch in range(train_epoch):
    ...
    EPOCH_LIST = [10,20,50,100]  # add epoch at which the image needs to be saved
    if epoch+1 in EPOCH_LIST:
        p = 'MNIST_GAN_results/results/MNIST_GAN_' + str(epoch + 1) + '.png'
        #p = 'C:\\Users\\Omkar\\PycharmProjects\\DLCV_CW2\\venv\MNIST_GAN_results\\results\\' + str(epoch + 1) + '.png'
